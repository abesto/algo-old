...
var StateUI = (function()
{
    ...
    return function($state, sort) {
        SAS.addSlot(function(signal) {
            try {
                if (signal.type() != 'update_bin_end') removeHl($state);
                eval(signal.type())($state, signal.allParams());
            } catch (E) {}
        }, sort.uid);
        ...
        this.setHistory = function(history) {
            SAS.addSlot(function(signal) {
                ...
                // Clear
                $state.find('.occurences').find('td, th').remove();
                $state.find('.bin-ends').find('td, th').remove();
                $state.find('.sorted li').html('');

                // Min-max
                if (state.minKey != undefined) {
                    if (signal.type() == 'minmax') removeHl($state);
                    minmax($state, state);
                }
                ...
                if (state.occurences.length > 0) {
                    for (i in state.occurences) {
                        if (signal.type() == 'occurence') removeHl($state);
                        occurence($state, {
                            key: i,
                ...
                if (state.binEnds != undefined) {
                    for (i in state.binEnds) {
                        if (signal.type() == 'bin_end') removeHl($state);
                        bin_end($state, {key: i, end: state.binEnds[i]});
                    }
                }
                ...
                if (state.sorted != undefined && state.sorted.length > 0) {
                    for (i in state.sorted) {
                        removeHl($state);
                        sorted($state, {idx: i, element: state.sorted[i]});
                    }
                }
            }, history.uid);
        };
